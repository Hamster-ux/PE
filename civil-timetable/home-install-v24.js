(function(){
  ...
  function showGuide(){
    ...
    if(deferredInstall){
      ...
      deferredInstall.userChoice.finally(()=>{deferredInstall=null;render()});
      return;
    }
    if(isIOS()&&isSafari()){
      alert('Safari 아래쪽 공유 버튼(네모에서 위로 화살표) → “홈 화면에 추가” → 오른쪽 위 “추가” 순서로 눌러 주세요.');
    }else if(isIOS()){
      alert('현재 화면을 Safari에서 연 뒤 공유 버튼 → “홈 화면에 추가”를 눌러 주세요. ChatGPT 앱 안 브라우저에서는 해당 메뉴가 보이지 않을 수 있어요.');
    }else{
  ...
  function addInstallCard(){
    ...
    if(!root||$('homeInstallCardV24'))return;
    const box=document.createElement('div');
    box.className='setting home-install-v24';
    box.id='homeInstallCardV24';
  ...
  settings=function(){previousSettings();addInstallCard()};
  const previousRender=render;
  render=function(){previousRender();addInstallCard()};
